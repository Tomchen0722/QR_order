...
def api_admin_upload():
    ...
    stem = Path(sec_name).stem or "image"
    filename = f"{stem}_{int(time.time())}{file_ext}"
...
def api_admin_menu_upsert():
    ...
    payload = {
        ...
        "is_available": data.get("is_available") in ("on", "1", True, 1),
...
def api_admin_menu_delete(item_id):
    method = (request.form.get("_method") or request.method).upper()
    if method != "DELETE":
...
def api_admin_tables_upsert():
    ...
    payload = {
        ...
        "name": name,
        "slug": slug,
        "is_active": data.get("is_active") in ("on", "1", True, 1),
    }
    conn = db.get_db()
    with conn:
